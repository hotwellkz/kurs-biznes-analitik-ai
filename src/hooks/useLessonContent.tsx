...
export const useLessonContent = (lessonId: string) => {
  ...
  const getPromptForLesson = (lessonId: string) => {
    switch (lessonId) {
      case '5.2':
        return 'Расскажи подробно как будто ты преподаватель и преподаеш Курс Бизнес Аналитик урок на тему: "Финальный проект Разработка полного набора документации для вымышленного проекта, Презентация проекта, Оценка: Завершение проекта"';
      case '5.1':
        return 'Расскажи подробно как будто ты преподаватель и преподаешь Курс Бизнес Аналитик урок на тему: "Кейсы реального мира Работа с примером реального проекта, Анализ требований и создание документации"';
      case '4.2':
        return 'Расскажи подробно как будто ты преподаватель и преподаешь Курс Бизнес Аналитик урок на тему: "Agile и Scrum Основы Agile, Роль бизнес-аналитика в Scrum-команде"';
      case '4.1':
        return 'Расскажи подробно как будто ты преподаватель и преподаешь Курс Бизнес Аналитик урок на тему: "Основы проектного менеджмента Основные понятия (scope, schedule, budget), Взаимодействие с командой проекта"';
      case '3.2':
        return 'Расскажи подробно как будто ты преподаватель и преподаешь урок на тему: "Моделирование процессов Основы BPMN (Business Process Model and Notation), Построение диаграмм процессов"';
      case '3.1':
